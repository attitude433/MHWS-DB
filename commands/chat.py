...
판단 애매하면 B 로 가서 가볍게.
- 현실 음식·동물·물건 이름이 와일즈 아이템과 겹쳐도, 질문자가 게임 맥락 없이 물으면 B. 예: "꽁치 알아?" = 일상 잡담(B), "꽁치 어디서 잡아?" = 와일즈 낚시(A).
...
TOOLS = [
    ...
    },
    {
        ...
        'description': '와일즈 무기별 모션치(모션값/배율)와 속성·상태이상 보정, 기절치, 부위파괴 보정 데이터를 가져옴. **질문에 "모션치/모션값/배율" 같은 말이 명시적으로 있을 때만** 사용할 것 (예: "대검 모아베기 모션치", "태도 기인참 배율 얼마", "쌍검 난무 모션값", "활 강사 속성보정"). "전적/기록/통계" 요청이나, 질문자 닉에 무기 이름이 들어있다는 이유만으로 이 도구를 쓰면 안 됨. weapon_kind는 대검/태도/한손검/쌍검/해머/수렵피리/랜스/건랜스/건랜스 포격/슬래시액스/차지액스/조충곤/라이트보우건/헤비보우건/활/고우키&파판 중 하나. attack에 공격명 일부를 넣으면 해당 공격만 필터, 비우면 그 무기 전체 목록.',
        'input_schema': {
            'type': 'object',
            'properties': {
                'weapon_kind': {'type': 'string', 'description': '무기 종류 한글명'},
    ...
    },
    {
        ...
        'description': '카톡방 멤버 한 명의 제니/룰렛/가위바위보 통계(=전적)를 가져옴. 출석 일수, 룰렛·가위바위보 횟수, 잭팟·초기화 횟수, 최대 단일 이득/손실, 최장 연승/연패, 평균·최대 베팅, 누적 베팅·환급, 현재 잔고·전체 순위. **"전적", "내전적", "내 전적", "전적 보여줘", "내 기록", "내 성적", "내 통계"** 같은 요청은 무조건 이 도구를 쓸 것 (무기 모션치 도구 아님 — 닉에 무기 이름이 붙어 있어도 헷갈리지 말 것). "내 잭팟 몇 번 떴어", "OO님 출석 며칠 했어", "내 최대 손실 얼마", "OO 룰렛 승률" 같은 질문에도 사용. 본인 질문이면 질문자 닉네임 그대로 넘기기.',
        'input_schema': {
            'type': 'object',
            'properties': {
