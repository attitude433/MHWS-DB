...
| `.출석` / `.출석체크` / `.출첵` | 1일 1회, 10~30 제니 균등 (시즌 잔고에) | 무료 |
| `.룰렛 [금액]` / `.룰렛 [%]` / `.룰렛 올` | 룰렛·가위바위보 합산 1일 3회, 10단계 확률표, 잭팟·초기화 방 공지, 퍼센트 베팅(1~100%, 소수점 올림). **베팅은 시즌 잔고에서** | 무료 |
| `.가위` / `.바위` / `.보 [금액]` / `[%]` | 가위바위보 베팅 (승 ×2.5 / 무 본전 / 패 몰수, EV +16.7%), 룰렛과 횟수 공유, 올 차단·100% 명시 시만 전액, 순수 random (pity 없음). **시즌 잔고에서** | 무료 |
...
- 다이애나 도메인용 웹 서버 (`commands/web.py`): `mhws.diana.ai.kr` (가비아 유료 .kr, 2026-06-08 등록) 으로 접속하면 제니 분포 대시보드 HTML 제공. Flask + Chart.js (CDN), 봇 프로세스 백그라운드 스레드로 0.0.0.0:80 listen. 디자인: 보라 그라데이션 + 글래스모피즘 + 골드 강조. 표시: 활성 멤버수 / 전체 제니 / 평균·중앙값 / 상위 5명 점유율 / 구간별 히스토그램 / 상위5 vs 나머지 도넛 / 로그 스케일 전체 랭킹 / 전체 랭킹 리스트(메달). 운영자(EXCLUDED) 및 닉네임 매핑 안 된 멤버 제외 (raw 토큰 노출 방지). OpenGraph 메타태그 포함 (카톡 미리보기). 인프라: Oracle Cloud Security List 인그레스 80 추가 + 인스턴스 iptables 80 ACCEPT + systemd unit 에 `AmbientCapabilities=CAP_NET_BIND_SERVICE` 추가 (ubuntu 유저로 80 bind).
- 분포 페이지 시스템 분석 카드 (펌프/풀림/큰 사건/60일 추이): `zenny_history` 일별 변화량 기반 누적 펌프(양수)·풀림(음수)·최대 단일 이득·최대 단일 손실·초기화 추정 횟수 stat + 60일 방 전체 총제니 라인 차트. 띵동빌런의 "0.9 안 도는 이유" = 인플레 추세 시각화.
- 분포 페이지 토큰 게이트 (`WEB_KEY` env): `?key=<TOKEN>` 일치해야 통과, 아니면 게이트 안내 페이지(OG 태그 포함). API 도 동일. 봇 `.제니분포` 응답이 토큰 박힌 URL 자동 생성. 토큰 노출 시 `.env` 의 `WEB_KEY` 갱신 + 봇 재시작으로 옛 링크 무효화.
- 카톡방 현재 멤버 자동 필터 (Iris `/query` API): `chat_rooms.active_member_ids` 를 매 페이지 요청 시 조회해서 카톡방 떠난 멤버 자동 제외. fetch 실패 시 None → 필터 비활성 fallback (안전). 운영 동기화 작업 불필요.
- `zenny_events` 테이블 + 봇 이벤트 로그 (`members.py record_event`): 단일 이벤트 한 줄씩 저장 (user_id, ts, kind=attend/roulette/rps, bet, payout, delta, outcome, balance_after). 룰렛 outcome 은 `jackpot`/`reset`/`r+25` 등 정확한 % 값. 가위바위보는 `rps_win_가위_vs_보` 등 손 조합 포함. 봇 동작 영향 0.
- 카톡 export 백필 스크립트 (`scripts/backfill_events.py`): 카톡 txt → 봇 응답 패턴 매칭 → `zenny_events` 채워넣음. v2 정확도: 잭팟·초기화는 봇 공지의 닉을 직접 매칭 (가장 신뢰), 일반 룰렛/출석/RPS 는 큐 매칭 + 거부 응답("이미 출석", "횟수 초과", "제니 부족" 등) 패턴으로 큐 꼬임 방지, 10분 timeout. 결과 919건 정확 매칭 (잭팟 4·초기화 7 카톡 export 와 100% 일치).
- 멤버 프로필 페이지 (`/u/<닉>`): 분포 페이지 랭킹의 닉 클릭 시 이동 (공개). 표시: 닉/순위/잔고 헤더 + stat 5(출석/룰렛/RPS/잭팟/초기화 정확 카운트) + 60일 잔고 추이 라인 + 룰렛 outcome 도넛 + 가위바위보 손 패턴 막대 + 큰 사건 4(최대 이득/손실/최장 연승/연패) + 베팅 패턴 4(평균/최대/누적 베팅/환급) + 일별 손익 막대(양수 초록·음수 빨강). 데이터 소스: `zenny_events` + `zenny_history`. 토큰 게이트 동일 적용.
- 다이애나 LLM 도구 추가 (13종 → 15종): `get_member_stats(nick)` 으로 카톡방 멤버 한 명의 정확한 통계(출석·룰렛·RPS 횟수, 잭팟·초기화, 최대 이득/손실, 연승/연패, 베팅 패턴), `get_jackpot_reset_history()` 로 카톡방 전체 잭팟·초기화 명단·날짜 조회. 시스템 프롬프트에 분포 페이지·멤버 프로필 페이지 인지 + 명령어 목록 갱신 (`.가위/.바위/.보`, `.제니그래프`, `.제니분포`, `.출첵` 추가). 페이지 URL 은 직접 발설 안 함 (`.제니분포` 명령으로 받게 안내).
- 멤버 프로필 페이지 도박 순손익 카드 (띵동빌런 제안): `zenny_events` 의 룰렛+가위바위보 delta 합 = 도박 순손익 (출석 제외). `_collect_user` 에 `attend_sum`/`roul_pnl`/`rps_pnl`/`gambling_pnl` 추가, 페이지에 새 카드 4개. 다이애나 `get_member_stats` 응답에도 한 줄. "도박 안 한 8·9위 vs 도박러" 명확히 보임.
- `.제니순위` 카톡 정확 위치 자동 접힘 (zero-width space 500 트릭): 카톡 접힘 임계 = 500자. 누적 헤더 직후에 `​` 500개 삽입 → 그 위치에서 정확히 접힘. 시즌 상위 10만 미리보기, 누적 본문은 전체보기. 메모리 `feedback_katalk_foldout.md` 에도 패턴 저장.
- 시즌 시스템 (`commands/season.py` 253줄): 매월 1일 KST 자정 자동 전환. `seasons` (id, start_ts, end_ts, title) + `season_results` (시즌 종료 시 1·2·3등 등 영구 스냅샷) 테이블. 봇 백그라운드 thread 가 1시간 단위로 깨어나 시즌 경계 체크.
- 시즌 잔고 분리 (옵션 C): `members.cumulative_zenny` 컬럼 추가. 매월 1일 전환 시 `cumulative_zenny += zenny; zenny = 0` → 시즌 잔고 0 리셋, 누적은 영구 보존. 룰렛/가위바위보 베팅은 시즌 잔고에서만 굴러감 (0 이면 베팅 불가, 출석으로 모아야). 코드 위치: `season.py end_current_season_and_start_new`.
- `.제니` 응답 시즌+누적 두 줄: "🎰 시즌 잔고 X / 📊 누적 Y (역대 합)". 누적 표시 = `cumulative_zenny + zenny` (역대 결과 + 현재 시즌 진행 = 현재 보유 합).
- `.제니순위` 시즌+누적 분리: 시즌 (zenny 기반) 상위 10 + 누적 (cumulative_zenny + zenny) 전체. 카톡에선 zero-width space 트릭으로 누적 헤더에서 자동 접힘.
- 분포 페이지 시즌 챔피언 박스 + 멤버 페이지 시즌 통계 카드: 분포 페이지 상단에 "🏆 {시즌명} 랭킹 (상위 10)" 박스 (시즌 = zenny 기준). 메인 분포·전체 랭킹은 누적 (cumulative + zenny) 기준. 멤버 페이지 헤더에 시즌·누적 잔고 둘 다 표시, "🏆 시즌 통계" 카드 (점수·순위·시즌 출석/룰렛/RPS/잭팟/초기화 카운트).
- 다이애나 시스템 프롬프트 `[시즌제]` 섹션 신설: 매월 1일 자동 전환, 시즌 잔고 0 리셋 + 누적 합산, 룰렛 시즌 잔고로 굴러감 (0 이면 불가), 시즌 ≠ 누적 정상. `get_member_stats` 응답이 시즌·누적 잔고 두 줄 + 시즌 순위 표시.
- 역대 시즌 1·2·3등 메달 표시: `season.py get_user_medals(user_id)` 헬퍼 (종료 시즌 중 그 유저 1·2·3등 기록). `_collect()` 에 `past_seasons`, `_collect_user()` 에 `medals` 추가. 분포 페이지 상단에 "🏅 역대 시즌 챔피언" 박스 (시즌별 한 줄), 멤버 페이지에 "🏅 시즌 우승 기록" 카드 (본인 메달). 6월 진행 중엔 데이터 0 → 박스 숨김 (display:none). 7월 1일 자동 전환 후 첫 시즌 종료 + `season_results` 스냅샷 → 즉시 표시.
